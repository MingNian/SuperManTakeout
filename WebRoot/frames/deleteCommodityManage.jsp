	...
	<head>
		...
		<script type="text/javascript">
			function toPage(pageNum){
				window.location.href="shop/preDelCommodity_CommodityAction?currPage="+pageNum;
			}
		</script>
	</head>
	<body>
		<div id="div0">
			<div id="div1">
				<table id="tbl" border="1" style="width:98%;">
					...
					<c:choose>
						<c:when test="${empty commodities3}">
						<tr>
						...
						<c:otherwise>
							<c:set var="i" value="0"/>
							<c:forEach items="${commodities3}" var="commodity">
								<c:set var="i" value="${i+1}"/>
								<tr style="text-align:center;">
									<td>${i}</td>
									<td><a href="delComDetail_CommodityAction?commodity.id=${commodity.id}">${commodity.commodityName}</a></td>
									<td>${commodity.category.categoryName}</td>
									<td>${commodity.commodityRemark}</td>
									<td>${commodity.price}</td>
									<td>
										<c:choose>
											<c:when test="${commodity.status==false}">
												<a href="addStatus_CommodityAction?commodity.id=${commodity.id}&commodity.status=${commodity.status}">重新上架</a>
											</c:when>
										</c:choose>
									</td>
								</tr>
